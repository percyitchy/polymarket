...
# This script verifies local code before comparing with remote server
...
# Detect checksum command (OS-agnostic)
...
# Use git ls-files to get all tracked files, then filter for relevant types
...
OUTPUT_FILE="local_checksums.txt"
...
# Header
echo "==========================================" >> "$OUTPUT_FILE"
echo "Local Code Checksums - $(date)" >> "$OUTPUT_FILE"
echo "==========================================" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
...
echo ""
echo "Next steps:"
echo "1. Upload verify_remote_code.sh to server"
echo "2. Run it on the server"
echo "3. Compare the checksums using: diff local_checksums.txt remote_checksums.txt"
